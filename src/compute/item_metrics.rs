...
use crate::{consts::VALVE_GAME_IDS, prices::{PriceValue, PriceValueTrait}, MarketItem};
...
impl ItemMetricCalculation for SteamEstimatedFee {
    fn calculate(&self, item: &MarketItem) -> ItemMetricValue {
        let steam_fee: PriceValue = item.history.iter()
...
impl ItemMetricCalculation for GameEstimatedFee {
    fn calculate(&self, item: &MarketItem) -> ItemMetricValue {
        let game_fee: PriceValue = item.history.iter()
